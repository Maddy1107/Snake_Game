...
#Button message
def message_button(msg,color,buttonx,buttony,button_width,button_height,size='small'):
    textscreen,textblock=text_block(msg,color,size)
    textblock.center= (buttonx+(button_width/2)),(buttony+(button_height/2))
    screen.blit(textscreen,textblock)
...
def start_screen():
    ...
    while start==True:
        ...
        button('PLAY',150,500,100,50,light_red,red,action = 'play')
        button('CONTROLS',320,500,150,50,light_yellow,yellow,action = 'controls')
        button('QUIT',550,500,100,50,light_blue,blue,action = 'exit')
...
def button(text,x,y,width,height,active_color,inactive_color,action=None):
    ...
    message_button(text,black,x,y,width,height,size='small')
